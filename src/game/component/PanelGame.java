...
public class PanelGame extends JComponent {
    ...
    private void initKeyboard() {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                float s = 0.9f; // 0.5f orj (ucak kendi etrafında dönüş)
                while (start) {
                    if (player.isAlive()) {
                    ...
                    for (int i = 0; i < rockets.size(); i++) {
                        ...
                        if (rocket != null) {
                            ...
                            if (!rocket.check(width, height)) {
                                ...
                                // System.out.println(" Jpanelden cıkınca Roket Removed .. testi");
                            } else {
                                if (player.isAlive()) {
                                    checkPlayer(rocket);
                                }
    ...
    private void checkBullets(Bullet bullet) {
        for (int i = 0; i < rockets.size(); i++) {
            ...
            if (rocket != null) {
                ...
                area.intersect(rocket.getRocketShap());
                if (!area.isEmpty()) {
                    boomEffects.add(new Effect(bullet.getCenterX(), bullet.getCenterY(), 200, 5, new Color(21, 155, 27), 7, 0.5f));
                    if (!rocket.updateHP(bullet.getSize())) { // For Test HP // test için olan if(true) silindi
                        score++;
                        rockets.remove(rocket);
                        double x = rocket.getX() + Rocket.ROCKET_SIZE / 2;
                        double y = rocket.getY() + Rocket.ROCKET_SIZE / 2;
                        boomEffects.add(new Effect(x, y, 270, 7, new Color(128, 24, 114), 7, 0.05f));
                        boomEffects.add(new Effect(x, y, 270, 7, new Color(47, 19, 121), 7, 0.1f));
                        boomEffects.add(new Effect(x, y, 350, 10, new Color(21, 145, 147), 10, 0.3f));
                        boomEffects.add(new Effect(x, y, 350, 7, new Color(182, 32, 100), 7, 0.05f));
                        boomEffects.add(new Effect(x, y, 270, 7, new Color(47, 19, 121), 7, 0.05f));
                    }
    ...
    private void checkPlayer(Rocket rocket) {
        if (rocket != null) {
            Area area = new Area(player.getRocketShap());
            area.intersect(rocket.getRocketShap());
            if (!area.isEmpty()) {
                double rocketHp = rocket.getHP();
                if (!rocket.updateHP(player.getHP())) { // For Test HP // test için olan if(true) silindi
                    rockets.remove(rocket);
                    double x = rocket.getX() + Rocket.ROCKET_SIZE / 2;
                    double y = rocket.getY() + Rocket.ROCKET_SIZE / 2;
                    boomEffects.add(new Effect(x, y, 270, 7, new Color(128, 24, 114), 7, 0.05f));
    ...
    private void drawGame() {
        ...
        if (!player.isAlive()) {
            String text = "OYUN BİTTİ SAFA";
            String textKey = "Devam Etmek İçin Enter'a Bas ...";
